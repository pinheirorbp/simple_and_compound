      ...
      for (C.p in C.part){
        ...
        if(allow.degeneration == F & R.p == C.p){
          ...
          while (D.int.finalmat.a < sum(dim(M.a))) { # While the dimensions of the null matrix was smaller then the original matrix, keep going
            P.a[outer(as.numeric(rowSums(finalmat.a)>0),as.numeric(colSums(finalmat.a)>0))==1] <- 0 #probabilities of cells with both row and column non empty are zero
            P.a.modif= P.a
            if(any(rowSums(finalmat.a)==0)&any(colSums(finalmat.a)==0)&sum(M.a > 0)<sum(dim(M.a))){# if linkage density < 1 and there are both empty rows AND empty columns
              P.a.modif[outer(as.numeric(rowSums(finalmat.a)>0),as.numeric(colSums(finalmat.a)>0), "+")!=0] <- 0
            }#probabilities of cells with row OR column non empty are zero
            sel <- sample(1:length(M.a), 1, prob = P.a.modif) # Sample an cell of M.a with probability P.a.modif
            finalmat.a[sel] <- 1
